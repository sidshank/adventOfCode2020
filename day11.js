...
const seatArrangementRows = seatArrangementString.split('\n').map(row => [...row]);

const maxRows = seatArrangementRows.length;
const maxColumns = seatArrangementRows[0].length;

// ================================ PART 1 ================================
function getImmediateAdjacenciesForSeatIndex({ row, column, maxRows, maxColumns }) {
  let indices = [];
  for (let r = -1; r < 2; r++) {
    for (let c = -1; c < 2; c++) {
      const rowIndex = row + r;
      if (rowIndex < 0 || rowIndex >= maxRows) continue;
...
function precomputeImmediateAdjacencies({ rowCount, columnCount, seatArrangement }) {
  let adjacencies = {};
  for (let r = 0; r < rowCount; r++) {
    for (let c = 0; c < columnCount; c++) {
      if (seatArrangement[r][c] === '.') continue; // Don't care about floor locations.
      adjacencies[`${r},${c}`] = getImmediateAdjacenciesForSeatIndex({ row: r, column: c, maxRows, maxColumns });
    }
  }
  return adjacencies;
}

const immediateAdjacencies = precomputeImmediateAdjacencies({
  rowCount: maxRows,
  columnCount: maxColumns,
  seatArrangement: seatArrangementRows
});
...
function areAllSeatsUnoccupied(seats, seatArrangement) {
  return seats.filter( ([row, column]) => seatArrangement[row][column] === '#').length === 0;
}

function areNOrMoreAdjacentSeatsOccupied({seats, seatArrangement, limit}) {
  return seats.filter( ([row, column]) => seatArrangement[row][column] === '#').length >= limit;
}

function doSeatingIteration({seatArrangement, adjacencies, adjacencyLimit}) {
  const newSeatArrangement = seatArrangement.map(row => [...row]);
  let changeCount = 0;
  for (let row = 0; row < maxRows; row++) {
    for (let column = 0; column < maxColumns; column++) {
      const currentSeat = seatArrangement[row][column];
      if (currentSeat === '.') continue;
      const adjacentSeats = adjacencies[`${row},${column}`];
      if (isSeatEmpty(currentSeat) && areAllSeatsUnoccupied(adjacentSeats, seatArrangement)) {
        newSeatArrangement[row][column] = '#'; // occupy seat.
        changeCount++;
      } else if (
        isSeatOccupied(currentSeat) && 
        areNOrMoreAdjacentSeatsOccupied({seats: adjacentSeats, seatArrangement, limit: adjacencyLimit})
      ) {
        newSeatArrangement[row][column] = 'L'; // vacate seat.
        changeCount++;
      }
    }
  }
  return {
    seatArrangement: newSeatArrangement,
    changeCount
  };
}

function findSeatsOccupiedAtStableOccupancy({seatArrangement, adjacencies, adjacencyLimit}) {
  let isStable = false;
  let inputSeatArrangement = seatArrangement;
  while (!isStable) {
    const {seatArrangement: iteratedSeatArrangement, changeCount} = doSeatingIteration({
      seatArrangement: inputSeatArrangement,
      adjacencies,
      adjacencyLimit,
    });
    inputSeatArrangement = iteratedSeatArrangement;
    isStable = changeCount === 0;
  }
  return [].concat.apply([], inputSeatArrangement).filter(seat => seat === '#').length;
}

console.log(`PART 1 -> Seats occupied at stable zenith = ${findSeatsOccupiedAtStableOccupancy({
  seatArrangement: seatArrangementRows,
  adjacencies: immediateAdjacencies,
  adjacencyLimit: 4,
})}`);

// ================================ PART 2 ================================

function getVisibleAdjacenciesForSeatIndex({ row, column, maxRows, maxColumns, seatArrangement }) {
  let indices = [];
  for (let r = -1; r < 2; r++) {
    for (let c = -1; c < 2; c++) {
      if (r === 0 && c === 0) continue;
      let rowIndex = row + r;
      let columnIndex = column + c;
      while (rowIndex >= 0 && rowIndex < maxRows && columnIndex >= 0 && columnIndex < maxColumns) {
        if (seatArrangement[rowIndex][columnIndex] !== '.') {
          indices.push([rowIndex, columnIndex]);
          break;
        }
        rowIndex += r;
...
function precomputeVisibleAdjacencies({ rowCount, columnCount, seatArrangement }) {
  let adjacencies = {};
  for (let r = 0; r < rowCount; r++) {
    for (let c = 0; c < columnCount; c++) {
      if (seatArrangement[r][c] === '.') continue; // Don't care about floor locations.
      adjacencies[`${r},${c}`] = getVisibleAdjacenciesForSeatIndex({ row: r, column: c, maxRows, maxColumns, seatArrangement });
    }
  }
  return adjacencies;
}

const visibleAdjacencies = precomputeVisibleAdjacencies({
  rowCount: maxRows,
  columnCount: maxColumns,
  seatArrangement: seatArrangementRows,
});

console.log(`PART 2 -> Seats occupied at stable zenith = ${findSeatsOccupiedAtStableOccupancy({
  seatArrangement: seatArrangementRows,
  adjacencies: visibleAdjacencies,
  adjacencyLimit: 5,
})}`);
