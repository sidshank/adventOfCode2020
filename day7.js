const bagContainmentRulesInput = `muted white bags contain 4 dark orange bags, 3 bright white bags.
...
const bagContainmentRules = bagContainmentRulesInput.split('\n');

function getContainmentRelationFromRule(rule) {
  const containerAndContained = rule.match(/^(?<containerBag>.*) bags contain (?<containedBags>.*)/);

  if (containerAndContained.groups.containedBags === "no other bags.") {
    return {
      container: containerAndContained.groups.containerBag,
      contained: [],
      containedCounts: [],
    };
  } else {
    const bagsContained = containerAndContained.groups.containedBags.split(", ");
    const bagTypesContainedWithCounts = bagsContained.reduce((acc, b) => {
      const matches = b.match(/(?<bagCount>\d+) (?<bagType>.*) bag/);
      return {
        ...acc,
        [matches.groups.bagType]: matches.groups.bagCount
      };
    }, {});
    return {
      container: containerAndContained.groups.containerBag,
      contained: Object.keys(bagTypesContainedWithCounts),
      containedCounts: Object.values(bagTypesContainedWithCounts),
    };
  }
}

const bagContainedToContainerMappings = bagContainmentRules.reduce((allContainedMappings, bagRule) => {
  const containmentRelation = getContainmentRelationFromRule(bagRule);
  const {container, contained} = containmentRelation;
  for (let containedBag of contained) {
    if (allContainedMappings[containedBag]) {
      allContainedMappings[containedBag].push(container)
    } else {
      allContainedMappings[containedBag] = [container];
    }
  }
  return allContainedMappings;
}, {})

function getSetOfBagsThatContain(bagType) {
  const containerBagTypes = bagContainedToContainerMappings[bagType] ? bagContainedToContainerMappings[bagType] : [];
  let setOfBagTypes = new Set(containerBagTypes);
  for (let containerBagType of containerBagTypes) {
    setOfBagTypes = new Set([...setOfBagTypes, ...getSetOfBagsThatContain(containerBagType)])
  }
  return setOfBagTypes;
}

const setOfBags = getSetOfBagsThatContain('shiny gold');
console.log(`Number of bag colours that can eventually contain a shiny gold bag is ${setOfBags.size}`);

const bagContainerToContainedMappings = bagContainmentRules.reduce((allContainerMappings, bagRule) => {
  const containmentRelation = getContainmentRelationFromRule(bagRule);
  const {container, contained, containedCounts} = containmentRelation;
  for (let i = 0; i < contained.length; i++) {
    const containedBag = contained[i];
    const containedBagCount = containedCounts[i];
    if (allContainerMappings[container]) {
      allContainerMappings[container][0].push(containedBag);
      allContainerMappings[container][1].push(Number(containedBagCount))
    } else {
      allContainerMappings[container] = [[containedBag], [Number(containedBagCount)]];
    }
  }
  if (contained.length === 0) {
    allContainerMappings[container] = [[], []];
  }

  return allContainerMappings;
}, {});

function getNumberOfBagsIn(bagType) {
  const [containedBags, bagCounts] = bagContainerToContainedMappings[bagType];
  let acc = 0;
  for (let i = 0; i < containedBags.length; i++) {
    acc += (bagCounts[i] + bagCounts[i]*getNumberOfBagsIn(containedBags[i]))
  }
  return acc;
}

console.log(getNumberOfBagsIn('shiny gold'));
