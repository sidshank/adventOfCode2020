const puzzleInputString = `(7 * (7 * 6 * 4 + 3)) * 3 + ((8 + 5 * 3 + 7 + 8) + 3 * (2 + 9 * 5 + 7 * 5 * 6) * 2 + 3 * 9) + (4 + 5 + 5 * 2 + 2) + ((5 + 6 + 9 + 6) + 3 + 4 * 6 * 5)
7 * 5
    ...
    if (currentChar === '+' || currentChar === '*') {
      if (operation !== null) {
        if (expressionValue === -1) {
          expressionValue = Number(lhs);
        }
        expressionValue = operation === '+'? expressionValue + Number(rhs) : expressionValue * Number(rhs);
      }
...
function evaluateWithPreferenceForSums(expression) {
  let modifiedExpression = expression;
  while(modifiedExpression.includes("+")) {
    const additionMatches = modifiedExpression.match(/\d+\+\d+/);
    const result = evaluatePlainExpression(additionMatches[0]);
    modifiedExpression = 
      modifiedExpression.slice(0, additionMatches.index)
     + result
     + modifiedExpression.slice(additionMatches.index + additionMatches[0].length);
  }
  if (modifiedExpression.includes('*')) {
    return evaluatePlainExpression(modifiedExpression);
  }
  return Number(modifiedExpression);
}

function eliminateBrackets(expression, expressionEvaluator) {
  let modifiedExpression = expression;
  while(modifiedExpression.includes("(")) {
    let index = 0;
    let bracketStart = -1;
    while (index < modifiedExpression.length) {
      const currentChar = modifiedExpression[index];
      if (currentChar === '(') {
        bracketStart = index;
      } else if (currentChar === ')') {
        break;
    ...
    const result = expressionEvaluator(modifiedExpression.slice(bracketStart + 1, index));
    modifiedExpression = 
      modifiedExpression.slice(0, bracketStart)
     + result
     + modifiedExpression.slice(index +1);
  }
  return modifiedExpression;
}

const totalSum = puzzleInput.reduce((acc, line) => {
  const lineWithoutBrackets = eliminateBrackets(line, evaluatePlainExpression);
  return acc + evaluatePlainExpression(lineWithoutBrackets);
}, 0);
console.log(totalSum);

const totalSumWithAdditionPrecedence = puzzleInput.reduce((acc, line) => {
  const lineWithoutBrackets = eliminateBrackets(line, evaluateWithPreferenceForSums);
  return acc + evaluateWithPreferenceForSums(lineWithoutBrackets);
}, 0);
console.log(totalSumWithAdditionPrecedence);