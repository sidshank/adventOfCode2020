...
const answersForEveryGroup = customsAnswers.split('\n\n').map(groupAnswers => 
  groupAnswers.replace(/\n/g, ' ').split(' ')
);

const sumOfUniqueAnswers = answersForEveryGroup.reduce((acc, answersForGroup) => {
  const uniqueAnswers = new Set(answersForGroup.join(''));
  return acc + uniqueAnswers.size;
}, 0);

console.log(`The sum of unique questions with an answer of YES per group, is: ${sumOfUniqueAnswers}`);

const getNumQuestionsWithAllAnswersYes = answersForGroup => {
  let intersection = new Set([...answersForGroup[0]]);
  for (let index = 1; index < answersForGroup.length; index++) {
    const questionsWithYes = new Set([...answersForGroup[index]]);
    intersection = new Set(
      [...intersection].filter(x => questionsWithYes.has(x)));
  }
  return intersection.size;
}

const sumOfAnswers = answersForEveryGroup.reduce((acc, answersForGroup) => {
  return acc + getNumQuestionsWithAllAnswersYes(answersForGroup);
}, 0);

console.log(`For questions where everyone in a group answered YES, the sum is: ${sumOfAnswers}`);