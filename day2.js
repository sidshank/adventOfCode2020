...
function getPolicy(policyPasswordString) {
  const matches = policyPasswordString.match(
    /^(?<first>\d+)-(?<second>\d+)\W(?<letter>\w)/
  );
  return {
    first: Number(matches.groups.first),
    second: Number(matches.groups.second),
    letter: matches.groups.letter,
  }
}

function getPassword(policyPasswordString) {
  const matches = policyPasswordString.match(/:\W(?<password>\w+)/);
  return matches.groups.password;
}

function doesPasswordSatisfyOldJobPolicy(password, policy) {
  const characterMap = {};
  ...
  if (characterMap[policy.letter] === undefined) return false;
  count = characterMap[policy.letter];
  return count >= policy.first && count <= policy.second;
}

function doesPasswordSatisfyPolicy(password, policy) {
  const isCharInFirstPosition = password[policy.first - 1] === policy.letter;
  const isCharInSecondPosition = password[policy.second - 1] === policy.letter;
  return isCharInFirstPosition ^ isCharInSecondPosition;
}

const numberOfValidPasswords = policyPasswordStrings.filter(policyPasswordString => {
  const password = getPassword(policyPasswordString);
  const policy = getPolicy(policyPasswordString);
  // return doesPasswordSatisfyOldJobPolicy(password, policy);
  return doesPasswordSatisfyPolicy(password, policy);
}).length;

console.log(`${numberOfValidPasswords} were valid`);
