...
function isNumberBetween(input, min, max) {
  const number = (typeof input === 'string') ? Number(input) : input;
  return !Number.isNaN(number) && (min <= number) && (number <= max);
}
...
const rules = {
  ...
  'ecl': ecl => eyeColours.has(ecl),
  'hcl': hcl => hcl.length === 7 && hcl.match(/#[0-9a-f]{6}/),
  'pid': pid => pid.length === 9 && pid.match(/\d{9}/),
  'cid': cid => true,
  'hgt': hgt => {
    const matches = hgt.match(/^(\d+)(in|cm)$/);
    if (matches === null) return false;
    const heightValue = Number(matches[1]);
...
function areFieldValuesValid(passportObject) {
  const keys = Object.keys(passportObject);
  const validFields = keys.filter(key => rules[key](passportObject[key]));
  return validFields.length === keys.length;
}

function isValidPassportWithValidFieldValues(passportObject) {
  return isValidPassport(passportObject) && areFieldValuesValid(passportObject);
}

function isValidPassport(passportObject) {
  return necessaryFields.filter(field => passportObject[field] !== undefined).length === necessaryFields.length;
}
...
const passports = passportData.split('\n\n').map(passport => 
  passport
    .replace(/\n/g, ' ')
    .split(' ')
    .map(kvPairString => kvPairString.match(/(\w+):(.*)/))
    .reduce((acc, kvPair) => ({...acc, [kvPair[1]]: kvPair[2]}), {})
);

const validPassports = passports.filter(isValidPassport);
console.log(`${validPassports.length} valid passports`);

const validPassportsWithValidFields = passports.filter(isValidPassportWithValidFieldValues);
console.log(`${validPassportsWithValidFields.length} valid passports WITH valid fields`);